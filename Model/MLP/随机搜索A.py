...
param_dist = {
    'hidden_layer_sizes': [
        (30, 30), (50, 30), (50, 50), (100, 50), (100, 100), 
        (30, 30, 30), (50, 50, 50), (100, 50, 30), (100, 100, 50)
    ],  # 增加更多隐藏层结构组合，包括三层隐藏层
    'activation': ['relu', 'tanh', 'logistic', 'identity'],  # 增加identity激活函数
    'solver': ['adam', 'lbfgs', 'sgd'],  # 优化算法
    'alpha': [0.00001, 0.0001, 0.001, 0.01, 0.1, 1],  # 增加更小和更大的L2正则化参数
    'learning_rate': ['constant', 'invscaling', 'adaptive'],  # 学习率策略
    'max_iter': [200, 300, 400, 500, 600, 800, 1000],  # 增加更大的最大迭代次数
    'batch_size': [8, 16, 32, 64, 128, 256]  # 增加更小和更大的批量大小
}
# 创建MLP模型
...
random_search = RandomizedSearchCV(
    ...
    n_iter=100,  # `n_iter` 用于指定随机搜索过程中要尝试的不同参数组合的数量，此处设定为100表示将尝试100种不同的参数组合
    cv=cv,
    scoring='neg_root_mean_squared_error',  # 使用RMSE（Root Mean Squared Error，均方根误差）作为评估指标。RMSE是预测值与真实值之间误差的平方和的平均值的平方根，它对较大的误差更为敏感，能有效反映模型预测值与真实值的偏离程度，在回归问题中常用于衡量模型的预测精度。负号是因为scikit-learn中默认优化方向是最大化评分，使用负的RMSE可以将最小化RMSE的问题转换为最大化负RMSE的问题。
    n_jobs=-1,  # 使用所有CPU核心
    verbose=2,  # 显示详细信息
    random_state=42
)
...
excel_path = os.path.join(save_dir, 'MLP随机搜索结果A.xlsx')
