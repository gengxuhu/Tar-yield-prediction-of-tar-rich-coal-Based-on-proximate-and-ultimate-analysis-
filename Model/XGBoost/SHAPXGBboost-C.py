...
model = XGBRegressor(
    ...
    subsample=0.6,        # 减少样本采样比例
    colsample_bytree=0.6, # 减少特征采样比例
    reg_alpha=0.1,        # 新增L1正则化
    reg_lambda=1,         # 新增L2正则化
    min_child_weight=2,   # 新增子节点最小权重
    random_state=114
)
...
# 将DataFrame写入Excel文件
with pd.ExcelWriter(r'D:\研究生学习\博士学习\论文3\数据集\模型参数\XGBoostC.xlsx') as writer:
...
# 添加标注并保存图形
ax_sw.text(0.02, 0.98, '(h)', transform=ax_sw.transAxes,
...
output_main_folder = r"D:\研究生学习\博士学习\论文3\绘图\shapXGboost\C"
