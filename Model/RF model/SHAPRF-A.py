...
# 将DataFrame写入Excel文件
with pd.ExcelWriter(r'D:\研究生学习\博士学习\论文3\数据集\模型参数\RFa.xlsx') as writer:
...
def plot_advanced_interaction(primary_feature_name, interacting_feature_name, x_values, interaction_feature_values,
    ...
    ax1.bar(bin_centers, counts, width=bin_width * 0.7, color='gray', alpha=0.2,
            label='Distribution')  # 在ax1上绘制分布直方图   
    ax1.set_ylabel('Distribution', ax_sw.tick_params(axis='x', labelsize=34))  # 设置ax1的y轴标签   
    ax1.set_ylim(0, counts.max() * 1.1)  # 设置ax1的y轴范围   
    ...
    # 使用工业分析标签映射设置X轴标签
    ax1.set_xlabel(industrial_analysis_labels.get(primary_feature_name, primary_feature_name), ax_sw.tick_params(axis='x', labelsize=34))
    ax2.set_ylabel(f'SHAP Interaction Value', ax_sw.tick_params(axis='x', labelsize=34))  # 设置ax2的y轴标签
    ...
    output_filename = f"interaction_{sanitized_primary}_vs_{sanitized_interacting}.svg"  # 构建输出文件名   
    full_path = os.path.join(save_folder, output_filename)  # 拼接完整保存路径   
