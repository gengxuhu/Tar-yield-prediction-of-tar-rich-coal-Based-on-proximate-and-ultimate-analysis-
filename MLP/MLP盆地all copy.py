...
# 图4：真实值 vs 预测值 (最后一次迭代)
last_pred = simulation_results['predictions'][-1]
axes[1, 1].scatter(y_test, last_pred, alpha=0.6, color='purple', label='预测点')
axes[1, 1].plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--', lw=2, label='1:1 线')
axes[1, 1].set_xlabel('真实值 (Tar)')
axes[1, 1].set_ylabel('预测值 (Tar)')
axes[1, 1].set_title('真实值 vs 预测值 (最后一次迭代)')
axes[1, 1].legend()

plt.suptitle(f'MLP 蒙特卡洛模拟评估报告 (N={N_SIMULATIONS}, Noise $\sigma$={NOISE_SIGMA})', fontsize=16)
...
print("\n保存结果中...")
output_path = 'D:\研究生学习\博士学习\论文3\数据集\模型参数\MLP盆地all_MonteCarlo.xlsx'
try:
    with pd.ExcelWriter(output_path) as writer:
        summary_df.to_excel(writer, sheet_name='统计汇总', index=False)
        # feat_df.to_excel(writer, sheet_name='特征重要性分析', index=False) # MLP没有特征重要性，注释掉
