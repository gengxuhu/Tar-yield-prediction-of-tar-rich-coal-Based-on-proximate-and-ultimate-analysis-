...
param_dist = {
    'hidden_layer_sizes': [ (50, 50), (100, 50), (100, 100), (50, 50, 50), (100, 100, 50), (150, 100), (200, 100)],  # 增加更多隐藏层结构组合
    'activation': ['relu', 'tanh', 'logistic', 'identity'],  # 增加激活函数
    'solver': ['adam', 'lbfgs', 'sgd'],  # 优化算法
    'alpha': [0.00001, 0.0001, 0.001, 0.01, 0.1, 1],  # 扩大L2正则化参数范围
    'learning_rate': ['constant', 'invscaling', 'adaptive'],  # 学习率策略
    'learning_rate_init': [0.0001, 0.001, 0.01, 0.1],  # 新增初始学习率参数
    'max_iter': [200, 400, 600, 800, 1000],  # 扩大最大迭代次数范围
    'batch_size': [8, 16, 32, 64, 128, 256],  # 扩大批量大小范围
    'momentum': [0.7, 0.8, 0.9, 0.95] if 'sgd' in ['adam', 'lbfgs', 'sgd'] else [],  # 新增动量参数（仅当solver包含sgd时）
    'nesterovs_momentum': [True, False] if 'sgd' in ['adam', 'lbfgs', 'sgd'] else [],  # 新增Nesterov动量参数（仅当solver包含sgd时）
    'early_stopping': [True, False],  # 新增提前停止参数
    'beta_1': [0.9, 0.95, 0.99] if 'adam' in ['adam', 'lbfgs', 'sgd'] else [],  # 新增adam的beta_1参数
    'beta_2': [0.999, 0.9995, 0.9999] if 'adam' in ['adam', 'lbfgs', 'sgd'] else []  # 新增adam的beta_2参数
}
# 创建MLP模型
...
excel_path = os.path.join(save_dir, 'MLP随机搜索结果B.xlsx')
