...
ax_sw = fig.add_axes([0.32, 0.11, 0.59, 0.8])  # 在图中添加一个子图(axes)用于绘制蜂群图(swarm plot)
...
ax_bar.barh(y=y_pos, width=shap_df["mean_shap"].values, height=0.7, color="blue", alpha=0.5, edgecolor="none",
            zorder=0)  # 绘制水平条形图，表示每个特征的平均重要性
...
ax_bar.set_xticklabels([f"{x:.2f}" for x in xticks_bar], fontsize=20)  # 设置条形图x轴的刻度标签格式
ax_bar.set_xlabel("Mean (|SHAP| value)", fontsize=20)  # 设置条形图x轴的标签
ax_bar.set_yticks(y_pos)  # 设置y轴刻度位置（与蜂群图共享）
# 修改条形图x轴刻度朝内
ax_bar.tick_params(axis='x', direction='in',length=4)  # 调整x轴刻度线的长度与数字大小
...
ax_sw.set_xticklabels([f"{x:.2f}" for x in sw_xticks], fontsize=20)  # 设置蜂群图x轴的刻度标签格式
# 修改蜂群图x轴名称为 importance，并修改x轴刻度朝内
ax_sw.set_xlabel("Relative importance", fontsize=20)  # 设置蜂群图x轴的标签
ax_sw.tick_params(axis='x', direction='in',length=4, labelsize=20)  # 调整x轴刻度线的长度与度字大小与数字大小
ax_sw.tick_params(axis='y', direction='in',length=4, labelsize=20)  # 调整y轴刻度线的长度与度字大小与数字大小
expl_main = shap.Explanation(  # 创建一个SHAP的Explanation对象，这是新版shap库绘图所需的标准格式
    values=shap_values_sorted,  # 排序后的SHAP值
    data=X_train_sorted.values,  # 排序后的特征值
...
ax_sw.set_yticks(y_pos)  # 设置y轴刻度位置# 在SHAP总览图绘制部分找到以下代码并修改
ax_sw.set_yticklabels([industrial_analysis_labels.get(f, f) for f in sorted_features], fontsize=20)  # 设置y轴刻度标签格式
ax_sw.tick_params(axis='y', direction='in',length=4)  # 调整y轴刻度线的长度与度字大小
...
# =======================================
# 在左上角添加 (a)
ax_sw.text(0.02, 0.98, '(b)', transform=ax_sw.transAxes,
           fontsize=20, va='top', ha='left',
           fontname='Times New Roman')
output_main_folder = r"D:\研究生学习\博士学习\论文3\绘图\shapRF\A"
combined_image_path = os.path.join(output_main_folder, 'combined_shap_summary_plot.SVG')  # 拼接组合图的保存路径
plt.savefig(combined_image_path, dpi=300, bbox_inches='tight')  # 保存图形
print(f"SHAP 特征重要性总览图已保存至: '{combined_image_path}'")  # 打印保存成功的提示
plt.show()  # 显示图形
print("\n正在绘制 SHAP 交互作用总览图...")  # 打印开始绘制交互作用图的提示
plt.figure()  # 创建一个新的图形窗口
shap.summary_plot(shap_interaction_values_sorted, X_train_sorted, max_display=10, show=False)  # 使用shap库绘制交互作用的摘要图
    ...
    if len(x_values) > 1:  # 检查样本数量是否足够
        ...
        if main_fit is not None and ci_data is not None:  # 如果成功计算
            ax2.plot(main_fit[:, 0], main_fit[:, 1], color='#9400D3', lw=2, label='LOWESS Fit',
                     zorder=4)  # 绘制主平滑曲线           
            ax2.fill_between(ci_data[0], ci_data[1], ci_data[2], color='#D3D3D3', alpha=0.15,
                             label='95%CI')  # 填充置信区间           
