...
param_dist = {
    'n_estimators': [50, 100, 150, 200, 300, 400, 500],  # 决策树数量，扩大范围至500
    'max_depth': [None, 5, 10, 15, 20, 25, 30],  # 树最大深度，扩大范围至30
    'min_samples_split': [2, 5, 10, 15, 20, 25],  # 分裂内部节点所需最小样本数，扩大范围
    'min_samples_leaf': [1, 2, 4, 6, 8, 10],  # 叶节点所需最小样本数，扩大范围
    'max_features': ['sqrt', 'log2', 0.3, 0.5, 0.7, 0.9],  # 构建树时考虑的特征数量，增加取值
    'bootstrap': [True, False],  # 是否使用自助采样
    'ccp_alpha': [0.0, 0.0001, 0.001, 0.01, 0.1, 0.2]  # 成本复杂度剪枝参数，增加取值
}
# 创建随机森林模型
...
excel_path = os.path.join(save_dir, 'RF随机搜索结果A.xlsx')
