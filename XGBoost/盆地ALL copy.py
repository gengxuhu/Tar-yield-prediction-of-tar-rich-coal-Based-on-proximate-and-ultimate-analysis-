...
try:
    ...
    # 为了演示代码逻辑，如果没有文件，这里可以生成随机数据（实际运行时请确保文件存在）
    # exit() 
    pass

# 提取特征和目标变量
# 假设特征列是 A 到 N (共14列)，目标是 Tar
# 注意：这里需要确保列名存在，如果不存在需要调整
...
simulation_results = {
    'mse': [], 'rmse': [], 'mae': [], 'r2': [],
    'train_r2': [],
    'predictions': [],  # 存储每次对测试集的预测结果 (N_samples, N_simulations)
    'feature_importances': []
}
...
for i in tqdm(range(N_SIMULATIONS), desc="Simulating"):
    ...
    # 3.4 预测
    # 训练集预测 (用于监控过拟合)
    x_pred = model.predict(X_train_noisy)
    # 测试集预测 (用于最终评估)
    ...
    simulation_results['train_r2'].append(train_r2)
...
predictions_matrix = np.array(simulation_results['predictions']).T # Shape: (n_samples, n_simulations)
feature_importances_matrix = np.array(simulation_results['feature_importances']) # Shape: (n_simulations, n_features)
...
# 图1：R² 和 MSE 的箱线图 (性能分布)
sns.boxplot(data=pd.DataFrame({'R²': r2_scores, 'MSE': mse_scores}), ax=axes[0, 0])
axes[0, 0].set_title('性能指标分布 (箱线图)')
axes[0, 0].grid(True, linestyle='--', alpha=0.6)

# 图2：R² 的时序波动图 (模拟过程中的稳定性)
axes[0, 1].plot(range(1, N_SIMULATIONS + 1), r2_scores, marker='o', markersize=3, linestyle='-', color='#1f77b4', alpha=0.8)
...
# 图3：预测结果的概率密度分布 (针对所有测试样本的预测均值)
# 这里展示测试集第一个样本的预测分布，或者所有R2的分布
sns.histplot(r2_scores, kde=True, ax=axes[1, 0], color='g', stat='density')
axes[1, 0].set_title('R² 分布概率密度 (KDE)')
axes[1, 0].set_xlabel('R²')

# 图4：特征重要性热力图 (Top 10 特征的变异性)
# 计算每个特征的均值和标准差
...
feat_cv = feat_std / (feat_mean + 1e-9) # 避免除零
...
sns.heatmap(heatmap_data_norm, annot=heatmap_data.values, fmt=".3f", cmap="YlGnBu", ax=axes[1, 1])
axes[1, 1].set_title('Top 10 特征重要性稳定性 (均值 vs 标准差)')

plt.suptitle(f'XGBoost 蒙特卡洛模拟评估报告 (N={N_SIMULATIONS}, Noise $\sigma$={NOISE_SIGMA})', fontsize=16)
...
print("\n保存结果中...")
output_path = 'D:\研究生学习\博士学习\论文3\数据集\模型参数\XGBoost_MonteCarlo_Eval.xlsx'
try:
