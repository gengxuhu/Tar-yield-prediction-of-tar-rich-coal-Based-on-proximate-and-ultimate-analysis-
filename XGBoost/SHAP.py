...
# 将DataFrame写入Excel文件
with pd.ExcelWriter(r'D:\研究生学习\博士学习\论文3\数据集\模型参数\XGBoostall.xlsx') as writer:
...
print("\n计算交互效应SHAP值（基于X_test）") #打印开始计算交互效应SHAP值的提示
shap_interaction_values = explainer.shap_interaction_values(X_test)     # 计算SHAP交互效应值，这是一个三维数组
print("SHAP交互效应值计算完成。")
...
def find_and_plot_crossings(ax, x_curve, y_curve, color, base_y_offset=0.9): # 定义一个函数，用于寻找并绘制曲线与y=0的交点（阈值）
    ...
    for i, k in enumerate(sign_changes): # 遍历所有符号变化点
         ...
         ax.text(x_root, y_text_position, f' {x_root:.2f} ', color='white', backgroundcolor=color, ha='center',
                 va='center', fontsize=9, bbox=dict(facecolor=color, edgecolor='none', pad=1)) # 在垂直线上方添加文本标签显示交点值
...
ax_bar.barh(y=y_pos, width=shap_df["mean_shap"].values, height=0.6, color="blue", alpha=0.5, edgecolor="none",zorder=0) # 绘制水平条形图，表示每个特征的平均重要性
...
shap.plots.beeswarm(expl_main, max_display=len(sorted_features), ax=ax_sw, show=False, plot_size=None) # 在指定的子图上绘制蜂群图
ax_sw.set_yticks(y_pos) # 设置y轴刻度位置ax_sw.set_yticklabels(sorted_features, fontsize=12) # 设置y轴刻度标签（特征名称）
...
# =======================================
output_main_folder = r"D:\研究生学习\博士学习\论文3\绘图\shap"
combined_image_path = os.path.join(output_main_folder, 'combined_shap_summary_plot.png') # 拼接组合图的保存路径
...
shap.summary_plot(shap_interaction_values_sorted, X_test_sorted, max_display=10, show=False) # 使用shap库绘制交互作用的摘要图
interaction_summary_plot_path = os.path.join(output_main_folder, 'shap_interaction_summary_plot.png') # 拼接交互作用图的保存路径
...
def plot_shap_dependence(feature_name, x_values, shap_values_for_feature, save_folder, custom_annotation=None): # 定义一个函数，用于绘制单个特征的SHAP依赖图 
    ...
    ax1.bar(bin_centers, counts, width=bin_width * 0.6, align='center', color='#4B0082', alpha=0.3, label='Distribution') # 在ax1上绘制分布直方图    
    ax1.set_ylabel('Distribution', fontsize=12) # 设置ax1的y轴标签    
    ax1.set_ylim(0, counts.max() * 1.1) # 设置ax1的y轴范围    
    ax2.scatter(x_values, shap_values_for_feature, alpha=0.3, s=25, color='#00008B', label='Sample', zorder=2) # 在ax2上绘制SHAP值的散点图
    if len(x_values) > 1: # 检查样本数量是否足够
        main_fit, ci_data = bootstrap_lowess_ci(x_values, shap_values_for_feature, frac=0.3) # 计算lowess平滑曲线和置信区间        
        if main_fit is not None and ci_data is not None: # 如果成功计算
            ax2.plot(main_fit[:, 0], main_fit[:, 1], color='#9400D3', lw=2, label='LOWESS Fit', zorder=4) # 绘制主平滑曲线            
            ax2.fill_between(ci_data[0], ci_data[1], ci_data[2], color='#D3D3D3', alpha=0.15, label='95%CI') # 填充置信区间            
...
def plot_advanced_interaction(primary_feature_name,interacting_feature_name,x_values,interaction_feature_values,shap_interaction_slice,save_folder):
    ...
    """
    sanitized_primary, sanitized_interacting = sanitize_filename(primary_feature_name), sanitize_filename(interacting_feature_name) # 清理主特征和交互特征的名称
    ...
    ax1.bar(bin_centers, counts, width=bin_width * 0.7, color='gray', alpha=0.2, label='Distribution') # 在ax1上绘制分布直方图    
    ax1.set_ylabel('Distribution', fontsize=12) # 设置ax1的y轴标签    
    ax1.set_ylim(0, counts.max() * 1.1) # 设置ax1的y轴范围    
...
output_folder_advanced_interactions = os.path.join(output_main_folder, 'advanced_interaction_plots_final') # 定义高级交互图的输出文件夹
...
for i in range(n_features): # 遍历所有特征，作为主特征    
    ...
    for j in range(n_features): # 再次遍历所有特征，作为交互特征        
        ...
        interaction_feature_values = X_test_sorted[interacting_feature_name] # 获取交互特征的数值（用于着色）        
        shap_interaction_slice = shap_interaction_values_sorted[:, i, j] * 2 # 获取这对特征的交互SHAP值（乘以2是为了与shap库的默认行为保持一致）        
